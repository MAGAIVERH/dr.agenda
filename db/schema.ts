...
export const usersToClinicsTableRelations = relations(usersToClinicsTable, ({ one }) => ({
  user: one(usersTable, { fields: [usersToClinicsTable.userId], references: [usersTable.id] }),
...
export const doctorsTable = pgTable('doctors', {
  ...
  availableFromTime: time('available_from_time').notNull(), //  (8:00 AM)
  availableToTime: time('available_to_time').notNull(), // (5:00 PM)
...
export const doctorsTableRelations = relations(doctorsTable, ({ one, many }) => ({
  ...
  appoitments: many(appointmentsTable),
}));
